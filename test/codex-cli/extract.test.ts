...
import { extractReview } from '@floor-agents/codex-cli'
import { MalformedReviewError } from '@floor-agents/codex-cli'
...
test('extracts from the header to the end, dropping the progress log', () => {
  ...
  expect(extractReview(raw)).toBe(['## Reviewer agent (Codex)', '', 'Looks fine.', 'Verdict: approve as-is'].join('\n'))
})

test('accepts the round-N header variant', () => {
  const raw = ['progress', '## Reviewer agent (Codex), round 3', 'Verdict: changes needed'].join('\n')

  expect(extractReview(raw)).toBe(['## Reviewer agent (Codex), round 3', 'Verdict: changes needed'].join('\n'))
})
...
test('finds the header even after a long progress log (never a head-based cut)', () => {
  ...
  expect(extractReview(raw)).toBe('## Reviewer agent (Codex)\nVerdict: approve with nits')
})
...
test('does not match a header mentioned mid-sentence without being at line start', () => {
  const raw = 'the text ## Reviewer agent (Codex) is just an example, no real header here'
