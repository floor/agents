...
| #89 | **The Project API and the control panel.** Every engine process serves a read-only API for its project (`/api/v1`: project and team, open issues with their runs, recorded runs, one run in full); `floor-agents serve` is the API alone. Floor IO's `/agents` reads it. A run now records its issue key, title and repository. `TaskAdapter.listOpenIssues()`, with Linear's state name, milestone and priority | the owner asked for a control panel, fed by an API, to understand the process. Building it found that **`LINEAR_PROJECT_ID` in the environment overrode the manifest's project**: the mtrl engine, started from the agents folder, listed the agents backlog, and a watcher would have run those issues against the mtrl repository. Fixed: the manifest wins. Also: Linear's cancelled state type was never matched |
