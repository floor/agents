...
**Conclusion:** a committee member must act when the event arrives. A headless CLI does; a GUI agent with no external push does not. Grok (a CLI) replaced the Antigravity GUI in the third seat. Gemini can now sit through the Antigravity CLI (`agy`, `provider: antigravity`) the same way. The GUI bridge scripts (`antigravity-relay.ts`, `antigravity-notify.ts`, `antigravity-mcp.ts`) remain in `scripts/` for reference.
