...
import { decideGate, DEFAULT_GATE_CONFIG, type GateDecisionInput } from '../../../packages/orchestrator/src/gate/decision.ts'
...
function approveComment(vendor: string, opts: { sha?: string; createdAt: Date; decision?: string } = { createdAt: new Date() }) {
...
test('custom config: a different needsHumanLabel is honored', () => {
  const result = decideGate(baseInput({
    ...
    config: { ...DEFAULT_GATE_CONFIG, needsHumanLabel: 'blocked-by-us' },
  }))
  expect(result.kind).toBe('hold')
})
...
test('custom config: a different authLabels set is honored', () => {
  const result = decideGate(baseInput({
    ...
    config: { ...DEFAULT_GATE_CONFIG, authLabels: ['security-sensitive'] },
    comments: [approveComment('codex', { sha: HEAD_SHA, createdAt: new Date() })],
  }))
