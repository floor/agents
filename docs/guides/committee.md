...
If an external agent disconnects mid-review, the gateway re-queues its task and re-dispatches on reconnect. If it times out entirely, the agent's vote counts as ABSTAIN. A review loop also stops before its third cycle when a member **repeats a blocker it had already written before the revision**: the implementer has had its try, and the point is usually one it cannot settle (a policy, a contract, a decision). The summary lists the blocker in both wordings under "Stood through a revision", the issue is labelled `needs-human`, and the way forward is written there: settle the point in a comment on the issue — reviewers read the discussion — then `floor-agents review --issue <id>`. "The same blocker" is measured as the share of significant words two blockers have in common (0.6 or more, same member); `BLOCKER:` lines are read whether written bold, as a heading (`## BLOCKER 1:`) or as a bullet. When failed seats leave a review with no decision, fix the cause and run `floor-agents review --issue <id>`: the committee is seated again on the same pull request, without another implementer turn. If its bridge answers only to report that its CLI failed (quota, login, crash), the seat is recorded as a failed execution: its PR comment is headed "did not review", the summary quotes the last line of the error, and nothing in that error is read as a vote or a blocker. If its bridge never starts, the engine abstains immediately rather than waiting for a poll timeout.
