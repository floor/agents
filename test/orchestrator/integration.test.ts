...
test('happy path: issue → LLM → branch → PR → status update', async () => {
  ...
  const orchestrator = createOrchestrator({
    ...
    taskAdapter,
    gitAdapter,
    llmAdapters: new Map([['anthropic', llmAdapter], ['lmstudio', llmAdapter], ['claude-code', llmAdapter]]),
    contextBuilder,
...
test('team mode: dev writes code, CTO approves', async () => {
  ...
  const orchestrator = createOrchestrator({
    ...
    taskAdapter,
    gitAdapter,
    llmAdapters: new Map([['anthropic', teamLLM], ['lmstudio', teamLLM], ['claude-code', teamLLM]]),
    contextBuilder,
    stateStore: createStateStore(STATE_DIR),
    costTracker: createCostTracker(),
  })
